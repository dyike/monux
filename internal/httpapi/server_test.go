...
func newTestServer(controller monitor.Controller, token string) *Server {
	switcher := service.NewSwitcher(controller, map[string]monitor.Input{"mac": 0x11, "linux": 0x0f})
	return New(switcher, token)
}
